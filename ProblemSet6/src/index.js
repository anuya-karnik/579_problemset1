...
function setup() {
    ...
    //shuffle the cards by adding the image class - lodash method
    //uses the Knuth Shuffle
    let imgArr = [1, 2, 3, 4, 5, 6, 7, 8, 9];
    let imgNumbers1 = _.shuffle(imgArr);
    let imgNumbers2 = _.shuffle(imgArr);

    let imgNumbers = imgNumbers1.concat(imgNumbers2);
    console.log(imgNumbers);

    for (let i = 0; i < cardArr.length; i++) {
        cardArr[i].classList.add('image' + imgNumbers[i]);
        cardArr[i].classList.add('cover');
...
function declareWin() {
    ...
    //text
    let winDisplay = document.createElement('SPAN');
    ...
    header.append(star1, winDisplay, star2);
